export class DataLoader {
    ...
    static processModItem(input) {
        ...
        try {
            ...
            if (addPayloadToArray && Array.isArray(existing)) {
                ...
                // Merge objects
                newValue = DataLoader.mergeObjects({ target: existing, source: payload, });
            } else {
                // Replace otherwise
                newValue = payload;
            }
            ...
            const leafHistoryNode = DataLoader.ensureNodePath({ node: modHistory, pathArray, modName });
    ...
    static addPayloadToHistory({ node, payload, modName }) {
        for (const [key, value] of Object.entries(payload)) {
            const nextChildren = DataLoader.ensureNodePath({ node, pathArray: [key], modName });
